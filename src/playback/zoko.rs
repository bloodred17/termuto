...
impl StreamProvider for ZokoProvider {
    ...
    async fn resolve(&self, request: &StreamRequest) -> Result<Option<Stream>> {
        ...
        Ok(Some(Stream {
            ...
            audio,
            quality,
            // This host serves its segments plainly; only the headers matter.
...
mod tests {
    use super::{
        Audio, PlayerConfig, Subtitle, ZokoProvider, decode_config, page_message, stream_page_url,
        subtitle_tracks, track_order,
    };
    use base64::Engine as _;
    use base64::engine::general_purpose::STANDARD;
